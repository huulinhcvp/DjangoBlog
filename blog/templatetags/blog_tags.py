...
def load_sidebar(user, linktype):
    ...
    # Tag cloud computing font size
    # Calculate the average value based on the total. The size is (number/average)*step size
    increment = 5
        ...
        if page_obj.has_previous():
            ...
            previous_url = reverse(
                ...
                kwargs={
                    'page': previous_number,
                    'tag_name': tag.slug})
    if page_type == 'Author\'s Article Archive':
        if page_obj.has_next():
            next_number = page_obj.next_page_number()
            next_url = reverse(
